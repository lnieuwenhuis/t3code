...
it.effect("returns actionable, transport-safe clone failures", () => {
  const cases: ReadonlyArray<{ stderr: string; expected: string; remoteUrl?: string }> = [
    ...
    {
      stderr: "fatal: early EOF https://user:secret@github.com/octocat/repo.git\n",
      expected: "fatal: early EOF https://github.com/octocat/repo.git",
    },
    {
      stderr: "fatal: an unrecognized clone failure\n",
      expected: "fatal: an unrecognized clone failure",
    },
  ] as const;
